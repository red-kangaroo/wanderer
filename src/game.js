...
// Map gen:
const mapWidth = 50;
const wallBreaks = 4; //higher reduces chance
...
var staves = ["",
              ...
              " of <t style='color:#"+rc()+"'>P</t><t style='color:#"+rc()+"'>u</t><t style='color:#"+rc()+"'>r</t><t style='color:#"+rc()+"'>e</t><t style='color:#"+rc()+"'> C</t><t style='color:#"+rc()+"'>h</t><t style='color:#"+rc()+"'>a</t><t style='color:#"+rc()+"'>o</t><t style='color:#"+rc()+"'>s</t>",];
...
const hints = ["You don't deserve forgiveness.",
               ...
               "It wasn't for their own good. Not really.",
               "Seek the levers once your Self glows again.",
               "Conquering your demons brings a certain boon.",
               "Have you lost your way?",
               "You might never be the same again...",
               "May you find that which you seek.",
               "Strike down anyone who stands in your path.",
               "Their blood shall nourish you.",
               "Your journey's end awaits in the distance.",
               "Nourish your Self or fade to oblivion.",
               "Some creatures here cannot be seen but from up close.",
               "Your enemies grow more ferocious when they can smell blood on you.",];

// Map tiles:
const emptyBlock = '&nbsp;';
const wallBlock = "<t style='background-color:#000000'>&#9608;</t>";
const tileWater = "<t style='background-color:#1E90FF'>~</t>";
const tileLava = "<t style='background-color:#FF4500'>~</t>";
const tileSludge = "<t style='background-color:#00FF00'>~</t>";
const tileBlood = "<t style='background-color:#8A0303'>~</t>";
const tileQuicksand = "<t style='background-color:#FFFF66; color:#DAA520;'>,</t>";
const tileIce = "<t style='background-color:#00BFFF; color:#FFFFFF;'>.</t>";
const tileBlindGas = "<t style='color:#696969'>#</t>";
const tileConfGas = "<t style='color:#FF1493'>#</t>";
const tileStunGas = "<t style='color:#FFFF00; font-weight: 900;'>#</t>";
const tileKey = "<t style='color:#800080'>9</t>";
const tileArmor = "<t style='color:#800080; font-weight: 900;'>]</t>"; //enchanted armor, normal is just ']'
const tileMobDoor = "<t style='background-color:#8B4513'>+</t>";
const tileGoldDoor = "<t style='background-color:#8B4513; color:#FFD700;'>+</t>";
const tileGoldWall = "<t style='color:#FFD700'>&#9608;</t>";
const tileStaff = "<t style='color:#800080; font-weight: 900;'>}</t>";
const tileTreasure = "<t style='color:#800080'>&#167;</t>";
const tileBramble = "<t style='color:#32CD32'>!</t>";
const tileNettles = "<t style='color:#DC143C'>!</t>";
const tileGrassTall = "<t style='color:#32CD32'>;</t>";
const tileGrassShort = "<t style='color:#32CD32'>.</t>";
const tileBloodDrop = "<t style='color:#8A0303'>.</t>";
const tileTreeStump = "<t style='color:#A52A2A; font-weight: 900;'>.</t>";
const tileTreeConif = "<t style='color:#006400; font-weight: 900;'>4</t>";
const tileTreeLeafy = "<t style='color:#32CD32'>8</t>";
const tileTreeDead = "<t style='color:#A52A2A; font-weight: 900;'>|</t>";
const tileBush = "<t style='color:#32CD32; font-weight: 900;'>%</t>";
//const tilePentagram = "<t style='color:#FF00FF'>&#9959;</t>"; //maybe use as summoning circle?
const tileMacGuffin = "<t style='color:#8B008B'>&#9765;</t>";
const tileSecretPath = "<t style='color:#85C1E9'>&#9608;</t>";
const tileTeleporter = "<t style='color:#00FFFF'>0</t>";

const impassable = [wallBlock,tileGoldWall,tileGoldDoor,tileMobDoor,tileTreeConif,tileTreeLeafy,tileTreeDead,tileBush];
const liquids = [tileBlood,tileLava,tileQuicksand,tileSludge,tileWater];
const gases = [tileBlindGas,tileConfGas,tileStunGas];
const plants_small = [tileBramble,tileNettles,tileGrassTall,tileGrassShort,tileTreeStump];
const plants_large = [tileTreeConif,tileTreeLeafy,tileTreeDead,tileBush];
const terrain = ['.',',','^','?','$',tileWater,tileGrassTall,[tileBramble,tileNettles,tileQuicksand,plants_large]];
const terrain_rare = ['/','_',']',tileTeleporter,tileKey,[tileSludge,tileBlood,gases]];
const terrain_any = [emptyBlock,wallBlock,'.',',','?','^',tileWater,tileLava,tileSludge,tileBlood,tileQuicksand,tileIce,tileBlindGas,tileConfGas,tileStunGas,tileGoldWall,tileBramble,tileNettles,tileGrassTall,tileGrassShort,tileBloodDrop,tileTreeStump,tileTreeConif,tileTreeLeafy,tileTreeDead,tileBush,tileSecretPath,tileTeleporter,['$','/',[']','_',tileKey,emptyBlock,wallBlock]]];
const treasures = ['$',']','?','_','/',tileKey,tileArmor,tileStaff,tileTreasure,tileMacGuffin];
...
var creatureList = [{icon:"<t style='color:#FF0000'>&</t>",
                     ...
                     aggro:4,
                     x:0,y:0,
                     flags: [["slow","boonphase"],["slow","boontime"],["slow","boonscore"],["slow","boonhaste"]][r(4)], // Random boon on death.
                     behaviour: function(){ if(!r(6)) currentMap[this.x][this.y]=wallBlock; return false; }
                    }];
const allFlags = ["diagonal","venom","toxic","flame","agony","regen","blind","trapper","teleself","teleother","knockback","flingback","steal","bloodpool","lavapool","toxicpool","waterpool","sandpool","icepool","smoke","chaos","disenchant","convoke","panic","paralysis","dodge","blooddeath","bloodattack","dispel","extend","extraattack","pierce","slow","esp","lifesaved","invulnerable","explode","phase","dig","boonphase","boontime","boonscore","boonhaste","dropscore","droppile","droptrap","worthless","confother","confgas","stunother","stungas","swap","jumpover","dumb","lazy","immobile","invisible"];
const attackVerbs = ["attacks","bashes","batters","bites","blasts","bleeds","buffets","chokes","claws","clobbers","clubs","crushes","cuts","disrupts","dissolves","freezes","gazes at","gores","headbutts","hits","impales","jabs","jolts","kicks","kisses","nibbles","pelts","pricks","pummels","punches","rends","rips","scratches","sears","shocks","slams","slaps","slashes","smacks","smashes","smites","splatters","squeezes","stabs","stings","stomps at","strangles","strikes","swats","thrashes","touches","tramples","whacks","whips","zaps"];
var currCre = [];
var mobBoss = {icon:"<t style='color:#8B008B'>X</t>",
               ...
               behaviour: function(){
                ...
                var k;
                // Change randomly:
                this.icon = this.icon.replaceAt(17, rc());
                ...
                if(stepsBetween(userPos,currCre[i])<2){
                 ...
                 if(checkEmpty(j,k) && inRangeCoords(j,k) && (j != 0 || k != 0)){
                  if(currentMap[this.x][this.y]==emptyBlock)
                    currentMap[this.x][this.y] = distanceBetween(userPos,this)>=10 ? tileTreasure : '$';
                  this.x = j;
                  this.y = k;
                  ...
                  if(distanceBetween(userPos,this)<5 && !r(4)){
                    ...
                    if(inRangeCoords(this.x+x,this.y+y) && checkEmpty(this.x+x,this.y+y) && checkEmpty(this.x+x+x,this.y+y+y) &&
                     ...
                     currCre.push(cloneCreature(mobOrb));
                     currCre[currCre.length-1].flags[0] = {x:x,y:y};
...
var mobOrb = {icon:"<t style='color:#800080'>*</t>",
              ...
              behaviour: function(){
                ...
                if(typeof(this.flags[0]) == "string")
                  return true;
                let i = currCre.indexOf(this);
...
var mobSawBlade = {icon:"<t style='color:#800000'>X</t>",
                   name:"<t style='color:#800000'>Giant Serrated Sawblade</t>",
                   attack:"shreds",
                   aggro:0,
                   x:0,y:0,
                   flags: ["tmp","worthless","invulnerable","bloodattack","knockback","dropnone"],
                   behaviour: function(){
                     if(typeof(this.flags[0]) == "string"){
                       this.flags[0] = Object.create({});
                       if(!r(2)){
                         this.flags[0].x = !r(2) ? 1 : -1;
                         this.flags[0].y = 0;
                       } else {
                         this.flags[0].x = 0;
                         this.flags[0].y = !r(2) ? 1 : -1;
                       }
                     }
                     ...
                     if(tryMove(this.x+this.flags[0].x,this.y+this.flags[0].y,i)){
                       doMove(this.x+this.flags[0].x,this.y+this.flags[0].y,i);
                     } else if(tryMove(this.x+this.flags[0].x * -1,this.y+this.flags[0].y * -1, i)){
                       this.flags[0].x = this.flags[0].x * -1;
                       this.flags[0].y = this.flags[0].y * -1;
                       doMove(this.x+this.flags[0].x,this.y+this.flags[0].y,i);
                     } else
                       this.flags[0] = "tmp";
                     return true;
...
function getMobBehaviour(){
 switch(r(300)){
  ...
  case 26:
   return function(){
     if(distanceBetween(userPos,this)<5 && !r(5)){
       msg += this.name+" curses you! ";
       if(hasted>0){
         msg += "You suddenly slow down. ";
         hasted = 0;
       } else if(!paralysed && !r(4)){
         // No stunlocking to death, so only allow paralysis when not already paralysed.
         msg += "You feel your muscles getting a little stiff. ";
         paralysed += r(4)+1;
       } else {
         let cantrips = [
           "You feel troubled. ",
...
// Random color:
function rc() { return Math.random().toString(16).substr(2,6); }
...
function newWorldName(){
 ...
 return "<t style='color:#"+rc()+"'>"+wName+"</t>";
}
...
function addLake(currStep=null){
 var stepsTaken = 0;
 var stepsFailed = 0;

 if(!currStep)
   currStep = { x:r(mapWidth), y:r(mapWidth) };
...
function addStructure(special_rooms=false){
 ...
 if((r(visited)>20) && (r(distanceToOrigin())>=10) && (r(kills)>20) && special_rooms){
  ...
  case 17:
    ...
    toAdd = ["&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;&#9608;",
      ...
      var randTerrain = "i2[$?7.':S;!)pP%~,6/0=bsq#dfn^34xmMaAc&Xw".split('');
  ...
  case 88:
   ...
   case 233:
    toAdd = ["WWWWWWWWWWWWWWWWWWWWWWWWWWWWW",
             ...
             "W..W..Wa:2::::A::::2:aW..W..W",
             "W..W..WW:::::::::::::WW..W..W",
             "WmmW..Wa::a:::a:::a::aW..WmmW",
             "W..WmmWW::::a:::a::::WWmmW..W",
             "W..W..Wa::::::i::::::aW..W..W",
             "W..W..WW:2:a:::::a:2:WW..W..W",
             "W..W..Wa::::::a::::::aW..W..W",
             "W..W..WW:a:a::a::a:a:WW..W..W",
             "W..W..WWWaWaW...WaWaWWW..W..W",
             "W..W++WWWWWWW+++WWWWWWW++W..W",
             "W..W..WaWaWaW...WaWaWaW..W..W",
             "W..Wm....m...M.M...m....mW..W",
             "W..W...m...m.....m...m...W..W",
             "WmmWaWaWaWaWWaAaWWaWaWaWaWmmW",
   ...
   case 254:
    toAdd = ["               ``  %%%%   ppp   ```         ",
             ...
             "      `%`%%a%```````WAW``.`WWWW```pp``%``%` ",
             "    `m%%%p%``WWWW``.+.W...`!``T````pppW%``  ",
             "  T`%%`%Tp``pWA.+...WWW.`!!!WWWWT```Wppp``  ",
             "%T`%mT%p%`pp`WWWW``.T...!!T`W.aW``ppWp`ppp  ",
             "%```%```ppp~~~~````...T``%!`Wa.W``Tppp`pp`` ",
             "%%%%`a`%pp~~~~~~`````.`%````WW+WT```WpTT%T` ",
             " %%```TT%T%~~~~~~`T``M.........`````Wpp%`%%%",
             "``%`%`%%`%T%%~~~```...`WWWW```````T```%T`%` ",
   ...
   case 281:
    toAdd = ["       ``m``%``%%T`%  %`%    `` %%```     ",
             ...
             "    pppppppT%%%`a%T%`%%`%%%`%%%%`T`%%%T%  ",
             "    pppappp`%`%``%``%``T`%``TT%%T``%`T`%%%",
             "   ppapppT%`T`%`T%%%`T%``T``%`%```T%TTa`%%",
             "  TappppppT%`%`m`%T``a```%`%``````%%``T%TT",
             "%`%%`p`p`pp``%``T``````7```7```%``%``%`%  ",
             "`%TT%`%Ta%`T%`T%````````mTm``````%``T```  ",
             "%T``%T`%`%%`%TTT`%```7`T414T`7``T%`T`%%`%`",
             " `T%m%`TT%`%%%T`%``T````mTm`````%`a%%%%```",
             "  %`%%`%`%``%T%``m```T`7```7```T`T``%%T`  ",
             "  TA`%%%T`a`%``TT%T````T`T``TT%%`T`%T`m`  ",
             "   %T%%%%`T`TT%``%%%T`%%TT%%`%A%T`%%`TT   ",
             "    %%``%`%T   %%`TT%T`T    `%%`T%%%`     ",
             "      `%`%  `   `%m`%        `%T`%        "];
   break;
   ...
   case 284:
    toAdd = ["      %%%%       %%%%%        ",
             ...
             "`%%%`%`%%T%`T%`%%T%A%`%%`%`%%%",
             "%`T%A%%`%%`%%`%`%666%`66%T%%%%",
             "%%%T%```66`A``T%66666666666%``",
             " %%```%6666666666666666666666 ",
             "%%6666666666666666666666666666",
             " 6666666666666666666T`%66666% ",
             " `66666666666%``66T``c``66%%% ",
             "%%%%``666%``%`%`%``%%%T`c%`%% ",
             "%%%A%%`%`%%T%T%`%%T%%`%A%%%%%%",
             " %%T%`%%%%%%c%`%%%%`%`%T%T`%%%",
             "  %%%`%`%T%`%`%`T%%c%`%%TT%%  ",
   ...
   case 291:
    toAdd = ["             sssssssss        ",
             ...
             "    sssssT........``ssssssss  ",
             "  ssssss.W+WWWWWWWW`sssssssss ",
             " sssssssmW.m.m+.i.W``ssssssss ",
             " sssssss.W+WWWWWWWW``sssssssss",
             "ssssssssmW.CCCWC$CWT`sssssssss",
             "ssssssss.WWWWWWW+WW``sssssssss",
             "sssssssss.......````ssssssssss",
             " sssssssss....Ts.``.sssssssss ",
             "  ssssssssss..sss..sssssssss  ",
             "  sssssssssss..s..ssssssss    ",
             "   sssssssssss...sssssss      ",
             "    sssssssssss..sssss        ",
             "     sssssssss...sss          ",
             "       ssssss...sss           ",
   ...
   case 295:
    toAdd = ["WW  WW  WW  WW  WW  WW",
             ...
             " ...W.m........m.W... ",
             " ..66666777+7766666.. ",
             "WW 666667Aa$a766666 WW",
             "WW 666667a$aA766666 WW",
             " ..6666677877766666.. ",
             " ...W............W... ",
             "WW..W............W..WW",
             "WW..Wa..........aW..WW",
   ...
   case 296:
    toAdd = ["           777           ",
             ...
             "  77....7.......7....77  ",
             " 77....777.....777....77 ",
             "77..m..777.m.m.777..m..7 ",
             "77......7.......7......77",
             "7..767..7..777..7..787..7",
             "7.77677.7.77177.7.77.77.7",
             "7.7aA67.7.7$$$7.7.7$$$7.7",
             "7.7%-a7.7.7$$$7.7.7$$$7.7",
             "7.77%77.7.77.77.7.77A77.7",
             "7..7%7..7..787..7..777..7",
             "7.......7.......7.......7",
             "77..M..777.m.m.777..m..77",
             " 77...77 77...77 77...77 ",
             "  7...7   77777   7...7  ",
             "    .               .    "];
   break;
   ...
   case 314:
    toAdd = ["                 WWWW...WW  ",
             "           WWWWWWWWWa...WWW ",
             "       WWWWWWWWWWWWWW...66WW",
             "    WWWWW66aaWWWWWWWa...66aW",
             "  WWWWWWW66666WWWWWa...m66aW",
             " WWWWWWW...m6WWWWWWW...m6aWW",
             "WWWWW....W.....WAWW...WWWWW ",
             "Wa66m...WWaWW........WWWWW  ",
             "Wa66...WWWWW66m.....WWWWW   ",
             "Wa66..WWWWW6666WaWWWWWW     ",
             "WWWW...WWWWWaaWWWWWWWW      ",
             " WWWW...aWWWWWWWWWWW        ",
             " WWWW...WWWWWWWWWW          ",
             "  WWWa...WWWWWWW            ",
             "   WWW...WWWW               "];
   ...
   case 319:
    toAdd = [" WW..WW..WWWWWWWWWWWWW..WW..WW ",
             "WW..........6666666..........WW",
             "W..c.......666WWW666.......c..W",
             "..c...c..6666WW1WW6666..c...c..",
             "W....c.6666WWWW^WWWW6666.c....W",
             "W...c.6666WWWWC$CWWWW6666.c...W",
             "......SSSS.m.+C$C+.m.SSSS......",
             "......SSSS.m.+C$C+.m.SSSS......",
             "W...c.6666WWWWC$CWWWW6666.c...W",
             "W....c.6666WWWW^WWWW6666.c....W",
 ...
 var randWall = [emptyBlock,wallBlock].sampleRec();
  ...
  for (var j = 0; j < currentLine.length; j++) {
   if(inRangeCoords(x+i,y+j)) {
    ...
    case "W":
     ...
     break;
    case "v": // random wall (per tile)
     currentMap[x+i][y+j] = [emptyBlock,wallBlock].sampleRec();
     break;
    case "V": // random wall (per room)
     currentMap[x+i][y+j] = randWall;
     break;
    case "~":
    ...
    case "%": // random plants
     ...
     break;
    case "`": // random grass
     currentMap[x+i][y+j] = [tileGrassShort,tileGrassShort,tileGrassTall].sampleRec();
     break;
    case "p": // small plants
     if(!r(10))
    ...
    case "c": // pre-defined mob
     ...
     currentMap[x+i][y+j] = '.';
     break;
    case "C": // pre-defined mob + treasure
     currCre.push(cloneCreature(randMob));
     currCre[currCre.length-1].x = x+i;
     currCre[currCre.length-1].y = y+j;
     currentMap[x+i][y+j] = '$';
    ...
    case "X": // ice block
     ...
     break;
    case "w": // sawblade
     currCre.push(cloneCreature(mobSawBlade));
     currCre[currCre.length-1].x = x+i;
     currCre[currCre.length-1].y = y+j;
     currentMap[x+i][y+j] = '.';
     break;
...
function addUser(){
 ...
 userName = "<t style='color:#"+rc()+"'>"+newName()+"</t>";
}
   ...
   for(var m = 0; m < currCre.length; m++){
    if(distanceBetween(userPos,currCre[m])<=3){
      if(currCre[m].flags.includes("boss")){
        ...
        msg += currCre[m].name+" seems momentarily bewildered. ";
      } else {
...
function playerTakeDamage(dmg,i=9999){
 ...
 // Extra damage when bloodied, capped at +50 damage.
 if(bloodied>0) dmg += Math.min(r(kills), 50);
 // Apply damage resistance, then deal the damage and show it in message.
...
function tryPlayerMove(x,y){
 ...
 if(currentMap[x][y]==tileMobDoor){
   if(getAdjacentMobs()>0 && !timestop){
     msg += "You have no time to haul the heavy door open with enemies so close. ";
     return false;
   }
   return true;
 }
...
function damageCreature(i){
 ...
 if(currCre[i].flags.includes("invulnerable")) {
   ...
   if(!currCre[i].flags.includes("dropnone"))
     currCre[i].flags.push("stun");
   // Because otherwise it's too easy to get locked in place by invulnerable enemy,
   // but we're special casing it to prevent stunned sawblades.
   return;
 }
...
function moveUser(x,y){
 ...
 if(haveWon){
  msg = "<t style='color:#FF1493'>You win!</t><br />Your final score is "+getFinalScore()+" after "+String(turn)+" turns. Maybe there is a meaning to everything, after all.<br />Press <code>F5</code> to play again.";
  displayMap();
  return;
 }
 ...
 if(score<0){
  if(staff==4){
   ...
   msg = "You fade away...<br />Your final score is "+getFinalScore()+" after "+String(turn)+" turns. Better luck next life!<br />Press <code>F5</code> to play again.";
   displayMap();
   return;
  }
 }
 ...
 switch(currentMap[userPos.x][userPos.y]){
  ...
  case '/': //lever
   ...
   if(r(20)){
     ...
     while(q && tried){
       ...
       if(inRangeCoords(testX,testY) && currentMap[testX][testY]==wallBlock){
   ...
   if(!didSomething){
     switch (r(4)) {
       case 0: case 1:
        addLake(userPos);
        currentMap[userPos.x][userPos.y]='\\';
        msg += "The very earth beneath your feet trembles. ";
       break;

       case 2:
        addStructure();
        currentMap[userPos.x][userPos.y]='\\';
        msg += "The reality seems to stutter for a second. ";
       break;
       ...
       default:
        ...
        currentMap[userPos.x][userPos.y]='\\';
        msg += "Something falls from above. ";
       break;
     }
   }
   break;
  ...
  case tileGoldDoor:
   if(keys > 0){
     ...
     playerTakeDamage(r(10)+Math.ceil(score/20));
     currentMap[userPos.x][userPos.y]=',';
     break;
   }
 ...
 if(timestop>0){
  timestop--;
   ...
   // First handle paralysis:
   while (paralysed > 0) {
     paralysed--;
     msg += "You are <t style='color:#FF0000'>paralysed</t>! ";
     moveCreatures();
   }
   // Then all mobs take their normal turn:
   moveCreatures();
 }
...
function onExplosion(i=Infinity){
 // Declare epicentre of the explosion. If it's not a mob exploding, the explosion
 // will be around the player.
 var self;
 if(i == Infinity)
   self = userPos;
 else
 ...
 for(var k=-1; k < 2; k++){
  for(var l=-1; l < 2; l++){
   if(inRangeCoords(self.x+k,self.y+l)){
    // Destroy some terrain types:
    switch (currentMap[self.x+k][self.y+l]) {
      case tileNettles:
       currentMap[self.x+k][self.y+l] = tileBloodDrop;
       break;

      case tileGrassTall: case tileBramble: case tileBush:
       currentMap[self.x+k][self.y+l] = tileGrassShort;
       break;

      case tileTreeConif: case tileTreeLeafy: case tileTreeDead:
       msg += "The tree shatters into a thousand splinters. ";
       currentMap[self.x+k][self.y+l] = tileTreeStump;
       break;

      case tileMobDoor:
       msg += "The door is blown off its hinges. ";
       currentMap[self.x+k][self.y+l] = '.';
       break;
      ...
      case tileBlindGas: case tileConfGas: case tileStunGas:
       ...
       currentMap[self.x+k][self.y+l] = emptyBlock;
       break;
    }
    // Deal damage to the player if they get caught in the explosion:
    if(userPos.x == self.x+k && userPos.y == self.y+l){
...
function moveCreatures(){
 for (var k = 0; k < currCre.length; k++) {
  ...
  if((dist <= currCre[k].aggro || currCre[k].flags.includes("esp")) &&
   ...
   else if(stepsBetween(userPos,currCre[k])==2 && r(3) &&
           !(getAdjacentMobs() || blinded || panicked || paralysed || stunned) &&
           !currCre[k].flags.includes("dumb") && !(glyph==8 && bloodied>0)){
     // When one step away from player, sometimes don't move and let the player come.
     // Dumb mobs and mobs enraged by player in a bloodied armour of the blood knight
     // don't care and will just charge in.
     continue;
   }
  ...
  if(currCre[k].flags.includes("lazy") && !currCre[k].flags.includes("stun")) continue;
...
function getAdjacentMobs(){
  let count = 0;
  for (let i = 0; i < currCre.length; i++) {
    if(stepsBetween(userPos,currCre[i])<=1) count++;
  }
  return count;
}

//DEBUG
function devRandCharacter(){
  score = 500-r(500);
  visited = r(20)+2;
...
function displayMap(highlight=""){
 ...
 var states = ((aflame>0) ? "<t style='color:#FF4500'>Aflame</t> " : "") +
              ...
              ((paralysed>0) ? "<t style='background-color:#FF0000'>Stiffening</t> " : "") +
    ...
    switch (keypress) {
        ...
        case 191:// An alternate question mark code, apparently.
         ...
         alert("WandereRL "+VERSION+"\n\n"+
               ...
               "Hold 'Shift' to outline the nearby area in green and the furthest you can currently reach in red.\n\n"+
               "Your journey's end awaits in the distance.\n"+
...
function initialise(){
 ...
 for (; creatureChar.length>0; ) {
  ...
  base.icon = "<t style='color:#"+rc()+"'>"+creatureChar.pop()+"</t>";
 ...
 var welcome = ["May you find that which you seek.",
                ...
                "There is a Castle far, far away. Find it.",];
 ...
 newMap(0,0);
 msg += "Welcome, Wanderer. "+welcome.sampleRec()+"<br /><br />Press <code>?</code> for help. Press <code>Ctrl</code> to quickly find the Wanderer. ";
 displayMap();
}
